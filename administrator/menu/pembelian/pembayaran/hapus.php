...
if ((string) $row->status_posting !== 'draft') {
    set_flash('error', 'Pembayaran yang sudah diposting tidak bisa dihapus.');
    header('Location: ' . $back_url);
    exit;
}
...
try {
    $row->delete();
    set_flash('success', 'Pembayaran pembelian berhasil dihapus.');
} catch (Throwable $e) {
    set_flash('error', 'Gagal menghapus pembayaran pembelian: ' . $e->getMessage());
}
