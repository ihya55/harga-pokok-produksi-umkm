...
function angka_decimal_faktur_beli($value): float
{
    return (float) preg_replace('/[^0-9.]/', '', (string) $value);
}

function qty_int_faktur_beli($value): int
{
    return max(0, (int) preg_replace('/[^0-9]/', '', (string) $value));
}

function persen_int_faktur_beli($value): float
{
...
function redirect_tambah_faktur_beli(): void
{
    header('Location: ' . admin_url('index.php?menu=pembelian/faktur/tambah'));
    exit;
}

function validasi_coa_kas_bank_faktur_beli(int $id_entitas, string $jenis_pembayaran, int $id_coa): bool
{
...
$diskon_persen = persen_int_faktur_beli($_POST['diskon_persen'] ?? 0);
$ppn_persen = persen_int_faktur_beli($_POST['ppn_persen'] ?? pajak_default_persen($id_entitas, 'PPN', 11.0));
$ada_biaya_kirim = !empty($_POST['ada_biaya_kirim']) ? 1 : 0;
$biaya_kirim = $ada_biaya_kirim ? angka_decimal_faktur_beli($_POST['biaya_kirim'] ?? 0) : 0;
...
if ($tanggal_faktur === '') {
    ...
    redirect_tambah_faktur_beli();
}

if ($id_penerimaan_pembelian <= 0) {
    set_flash('error', 'Penerimaan pembelian wajib dipilih.');
    redirect_tambah_faktur_beli();
}
...
if ($sudah_ada_faktur) {
    set_flash('error', 'Penerimaan pembelian ini sudah pernah dibuatkan faktur.');
    redirect_tambah_faktur_beli();
}

if ($id_pemasok <= 0) {
    set_flash('error', 'Pemasok wajib dipilih.');
    redirect_tambah_faktur_beli();
}
...
if (($jenis_pembayaran === 'tunai' || $jenis_pembayaran === 'transfer') && !validasi_coa_kas_bank_faktur_beli($id_entitas, $jenis_pembayaran, $id_coa_kas_bank)) {
...
try {
    ...
    Capsule::connection()->transaction(function () use (
        ...
        foreach ($detail as $baris) {
            ...
            $qty = qty_int_faktur_beli($baris['qty'] ?? 0);
            $harga = angka_decimal_faktur_beli($baris['harga'] ?? 0);
            $diskon_detail = angka_decimal_faktur_beli($baris['diskon'] ?? 0);
        ...
        $no_faktur = generate_kode_master(
            'tb_faktur_pembelian',
            'no_faktur_pembelian',
            'FPB',
            4,
        ...
        $header = FakturPembelianORM::create([
            'id_entitas'              => $id_entitas,
            'no_faktur_pembelian'     => $no_faktur,
        ...
        $id_baru = (int) $header->id_faktur_pembelian;

        foreach ($detail_valid as $item) {
            FakturPembelianDetailORM::create([
                'id_faktur_pembelian' => $id_baru,
    ...
    set_flash('success', 'Faktur pembelian berhasil disimpan.');
    header('Location: ' . admin_url('index.php?menu=pembelian/faktur/detail&id=' . (int) $id_baru));
    exit;
} catch (Throwable $e) {
