...
$id_saldo_awal_stok = (int) ($_POST['id_saldo_awal_stok'] ?? 0);
...
function parse_number_saldo_awal_update($value): float
{
...
if ($tanggal_saldo_awal === '' || $id_gudang <= 0 || $id_coa_lawan <= 0) {
    ...
    header('Location: ' . admin_page_url('persediaan/saldo-stok/saldo-awal/edit') . '&id=' . $id_saldo_awal_stok . '&back_url=' . urlencode($back_url));
    exit;
}
...
try {
    Capsule::connection()->transaction(function () use (
        ...
        if ((string) $header->status_posting !== 'draft') {
            throw new Exception('Saldo awal stok yang sudah posted tidak bisa diubah.');
        }

        $header->tanggal_saldo_awal = $tanggal_saldo_awal;
        $header->id_gudang = $id_gudang;
        $header->id_coa_lawan = $id_coa_lawan;
        ...
        foreach ($jenis_barang_arr as $i => $jenis_barang) {
            ...
            $qty_awal = parse_number_saldo_awal_update($qty_awal_arr[$i] ?? 0);
            $harga_satuan = parse_number_saldo_awal_update($harga_satuan_arr[$i] ?? 0);
            ...
            SaldoAwalStokDetailORM::create([
                'id_saldo_awal_stok' => $id_saldo_awal_stok,
