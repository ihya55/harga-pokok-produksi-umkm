...
function kp_next_no(int $id_entitas): string {
    $prefix='KVP-'.date('ym').'-';
    $last=Capsule::table('tb_konversi_produk')->where('id_entitas',$id_entitas)->where('no_konversi_produk','like',$prefix.'%')->orderBy('id_konversi_produk','desc')->first();
    $n=1; if($last && preg_match('/(\d+)$/',(string)$last->no_konversi_produk,$m)) $n=((int)$m[1])+1;
...
try {
    ...
    Capsule::table('tb_konversi_produk')->insert([
        'id_entitas'=>$id_entitas,'no_konversi_produk'=>kp_next_no($id_entitas),'tanggal_konversi'=>$tanggal,'id_gudang'=>$id_gudang,
        'id_produk_sumber'=>$src,'qty_sumber'=>$qty_src,'hpp_sumber'=>$hpp_sumber,'nilai_sumber'=>$nilai_sumber,
        'id_produk_tujuan'=>$dst,'qty_tujuan'=>$qty_dst,'hpp_tujuan'=>$hpp_tujuan,'nilai_tujuan'=>$nilai_tujuan,
        'status_posting'=>'draft','catatan'=>$catatan,'tanggal_dibuat'=>date('Y-m-d H:i:s'),'dibuat_oleh'=>$id_pengguna?:null
    ]);
    set_flash('success','Konversi produk berhasil disimpan sebagai draft.');
