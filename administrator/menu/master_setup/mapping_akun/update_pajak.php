<?php
declare(strict_types=1);

require_once __DIR__ . '/../../../../helpers/config.php';
require_once __DIR__ . '/../../../../helpers/koneksi.php';
require_once __DIR__ . '/../../../../helpers/fungsi.php';
require_once __DIR__ . '/../../../../helpers/auth.php';
require_once __DIR__ . '/../../../../orm/PajakORM.php';

harus_login();

if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect_admin('master_setup/mapping_akun&tab=pajak_ppn');
}

$user = user_login();
$id_entitas = (int) ($user['id_entitas'] ?? 0);
$id_pengguna = (int) ($user['id_pengguna'] ?? 0);
$id_pajak = (int) ($_POST['id_pajak'] ?? 0);
$kode_pajak = strtoupper(trim((string) ($_POST['kode_pajak'] ?? 'PPN')));
$nama_pajak = trim((string) ($_POST['nama_pajak'] ?? 'Pajak Pertambahan Nilai'));
$persentase = (float) ($_POST['persentase'] ?? 0);
$status_aktif = (int) ($_POST['status_aktif'] ?? 1);
$keterangan = trim((string) ($_POST['keterangan'] ?? ''));

if ($id_entitas <= 0) {
    set_flash('error', 'Entitas pengguna tidak valid.');
    redirect_admin('master_setup/mapping_akun&tab=pajak_ppn');
}

if ($kode_pajak === '' || $nama_pajak === '') {
    set_flash('error', 'Kode dan nama pajak wajib diisi.');
    redirect_admin('master_setup/mapping_akun&tab=pajak_ppn');
}

if ($persentase < 0 || $persentase > 100) {
    set_flash('error', 'Persentase pajak harus antara 0 sampai 100.');
    redirect_admin('master_setup/mapping_akun&tab=pajak_ppn');
}

$row = null;
if ($id_pajak > 0) {
    $row = PajakORM::query()->where('id_entitas', $id_entitas)->find($id_pajak);
}
if (!$row) {
    $row = PajakORM::query()
        ->where('id_entitas', $id_entitas)
        ->where('kode_pajak', $kode_pajak)
        ->where('is_default', 1)
        ->first();
}
if (!$row) {
    $row = new PajakORM();
    $row->id_entitas = $id_entitas;
    $row->tanggal_dibuat = date('Y-m-d H:i:s');
    $row->dibuat_oleh = $id_pengguna > 0 ? $id_pengguna : null;
}

$defaultQuery = PajakORM::query()
    ->where('id_entitas', $id_entitas)
    ->where('kode_pajak', $kode_pajak);
if (!empty($row->id_pajak)) {
    $defaultQuery->where('id_pajak', '<>', (int) $row->id_pajak);
}
$defaultQuery->update(['is_default' => 0]);

$row->kode_pajak = $kode_pajak;
$row->nama_pajak = $nama_pajak;
$row->persentase = round($persentase, 2);
$row->is_default = 1;
$row->status_aktif = $status_aktif === 1 ? 1 : 0;
$row->keterangan = $keterangan !== '' ? $keterangan : null;
$row->tanggal_diubah = date('Y-m-d H:i:s');
$row->diubah_oleh = $id_pengguna > 0 ? $id_pengguna : null;
$row->save();

set_flash('success', 'Pengaturan PPN default berhasil disimpan di Mapping Akun.');
redirect_admin('master_setup/mapping_akun&tab=pajak_ppn');
