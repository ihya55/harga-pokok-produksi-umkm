...
if (!Capsule::schema()->hasTable('tb_payment_gateway_setting')) {
    set_flash('error', 'Tabel tb_payment_gateway_setting belum ada. Jalankan SQL update Duitku terlebih dahulu.');
    redirect_admin('master_setup/mapping_akun&tab=payment_gateway');
}
...
$merchant_code = trim((string) ($_POST['merchant_id'] ?? ''));
$api_key = trim((string) ($_POST['server_key'] ?? ''));
$enabled_payments = strtoupper(trim((string) ($_POST['enabled_payments'] ?? 'SP')));
$enabled_payments = $enabled_payments !== '' ? $enabled_payments : 'SP';

try {
    if ($is_active === 1 && ($merchant_code === '' || $api_key === '')) {
        throw new RuntimeException('Merchant Code dan API Key wajib diisi jika Duitku diaktifkan.');
    }
    ...
    $data = [
        ...
        'server_key' => $api_key,
        'client_key' => null,
        'merchant_id' => $merchant_code !== '' ? $merchant_code : null,
    ...
    set_flash('success', 'Setting Duitku QRIS dinamis berhasil disimpan.');
} catch (Throwable $e) {
