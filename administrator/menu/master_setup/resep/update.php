...
if ($id_produk <= 0 || !ProdukORM::query()->where('id_entitas', $id_entitas)->where('id_produk', $id_produk)->exists()) {
    ...
    redirect_admin('master_setup/resep/edit&id=' . $id_resep);
}
...
try {
    Capsule::connection()->transaction(function () use ($row, $id_entitas, $id_pengguna, $id_resep, $id_produk, $nama_resep, $jumlah_hasil, $versi_resep, $status_aktif, $detail) {
        $row->id_produk      = $id_produk;
        $row->nama_resep     = $nama_resep;
        $row->jumlah_hasil   = $jumlah_hasil;
        $row->versi_resep    = $versi_resep !== '' ? $versi_resep : null;
        $row->status_aktif   = $status_aktif;
        $row->tanggal_diubah = date('Y-m-d H:i:s');
        $row->diubah_oleh    = $id_pengguna > 0 ? $id_pengguna : null;
        $row->save();

        ResepDetailORM::query()
    ...
    set_flash('success', 'Data resep berhasil diperbarui.');
    redirect_admin('master_setup/resep');
} catch (Throwable $e) {
