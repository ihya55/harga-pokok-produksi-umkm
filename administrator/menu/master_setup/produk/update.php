...
if ($nama_produk === '') {
    ...
    redirect_admin('master_setup/produk/edit&id=' . $id_produk);
}
...
if ($barcode_produk_input !== '' && barcode_produk_sudah_dipakai($barcode_produk_input, $id_entitas, $id_produk)) {
...
$barcode_produk = $barcode_produk_input !== ''
    ...
    : generate_barcode_produk_otomatis($id_entitas, $id_produk);

$gambar_produk = $_POST['gambar_produk_lama'] ?? null;
...
if (isset($_FILES['gambar_produk']) && ($_FILES['gambar_produk']['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_NO_FILE) {
    ...
    if (!empty($gambar_produk) && file_exists($uploadDir . $gambar_produk)) {
...
$row->barcode_produk       = $barcode_produk;
$row->id_kategori_produk   = $id_kategori_produk;
$row->id_satuan            = $id_satuan;
$row->jenis_produk         = $jenis_produk;
$row->harga_jual           = $harga_jual;
$row->hpp_standar          = $hpp_standar;
$row->stok_minimum         = $stok_minimum;
$row->berat_bersih         = $berat_bersih;
$row->satuan_berat_bersih  = $satuan_berat_bersih;
$row->gambar_produk        = $gambar_produk;
$row->status_produk        = $status_produk;
$row->id_coa_penjualan     = $id_coa_penjualan;
$row->id_coa_hpp           = $id_coa_hpp;
$row->id_coa_persediaan    = $id_coa_persediaan;
$row->tanggal_diubah       = date('Y-m-d H:i:s');
$row->diubah_oleh          = $id_pengguna > 0 ? $id_pengguna : null;
$row->save();

set_flash('success', 'Data produk berhasil diperbarui.');
redirect_admin('master_setup/produk');