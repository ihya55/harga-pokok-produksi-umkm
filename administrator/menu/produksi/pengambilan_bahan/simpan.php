...
function angka_decimal_pengambilan_bahan($value): float
{
    return (float) preg_replace('/[^0-9.]/', '', (string) $value);
}


function angka_int_pengambilan_bahan($value): int
{
    return (int) preg_replace('/[^0-9]/', '', (string) $value);
}

function redirect_back_pengambilan_bahan(string $back_url): void
{
...
if ($tanggal_pengambilan === '') {
    ...
    redirect_back_pengambilan_bahan($back_url);
}
...
if (!is_array($id_bahan_baku_arr) || count($id_bahan_baku_arr) === 0) {
    set_flash('error', 'Detail bahan wajib diisi.');
    redirect_back_pengambilan_bahan($back_url);
}
...
foreach ($id_bahan_baku_arr as $i => $id_bahan_baku) {
    ...
    $qty = angka_int_pengambilan_bahan($qty_arr[$i] ?? 0);
    $harga_satuan = angka_decimal_pengambilan_bahan($harga_arr[$i] ?? 0);
...
try {
    ...
    Capsule::connection()->transaction(function () use (
        ...
        $no_pengambilan = generate_kode_master(
        ...
        $row = PengambilanBahanORM::create([
            'id_entitas'             => $id_entitas,
            'no_pengambilan_bahan'   => $no_pengambilan,
        ...
        $id_baru = (int) $row->id_pengambilan_bahan;

        foreach ($detail_valid as $detail) {
            PengambilanBahanDetailORM::create([
                'id_pengambilan_bahan' => $id_baru,
    ...
    set_flash('success', 'Pengambilan bahan berhasil disimpan sebagai draft. Silakan periksa stok sebelum posting.');
    header('Location: ' . admin_url('index.php?menu=produksi/pengambilan-bahan/detail&id=' . $id_baru . '&back_url=' . urlencode($back_url)));
    exit;
} catch (Throwable $e) {
