...
function kode_template_biaya_produksi_normalisasi_update(string $kode): string
{
    $kode = trim($kode);
...
function template_biaya_produksi_db_update(int $id_entitas, string $kode_jenis_transaksi, int $id_coa_kredit = 0): ?object
{
...
foreach ($jenis_detail as $i => $kode_template_input) {
    $kode_jenis_transaksi = kode_template_biaya_produksi_normalisasi_update((string) $kode_template_input);
    ...
    $template = template_biaya_produksi_db_update($id_entitas, $kode_jenis_transaksi, $id_coa_lawan);
