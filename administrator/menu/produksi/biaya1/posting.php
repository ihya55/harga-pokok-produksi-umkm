...
function kode_template_biaya_produksi_posting(string $kode): string
{
    $kode = trim($kode);
...
function nominal_template_biaya_produksi_posting(object $dt, object $detail, object $header): float
{
    $sumber = strtolower(trim((string) ($dt->sumber_nominal ?? 'total_biaya')));
    $rumus = strtolower(trim((string) ($dt->rumus_nominal ?? '')));

    if ($sumber === 'rumus_nominal') {
        $sumber = $rumus;
    }

    switch ($sumber) {
        case 'jumlah_detail':
        case 'jumlah_biaya_detail':
        case 'jumlah_biaya':
        case 'total_biaya':
            return round((float) $detail->jumlah_biaya, 2);

        case 'total_header':
        case 'total_biaya_header':
            return round((float) $header->jumlah_biaya, 2);
...
try {
    Capsule::connection()->transaction(function () use (
        ...
        foreach ($detail_rows as $detail) {
            ...
            $kode_log[] = $kode_template;

            foreach ($detail_template as $dt) {
                $jumlah = nominal_template_biaya_produksi_posting($dt, $detail, $row);
                ...
                if ((string) $dt->posisi_dc === 'debit') {
                    ...
                    throw new RuntimeException('Posisi debit/kredit template tidak valid.');
                }

                $id_coa = (int) $dt->id_coa_default;

                if ($id_coa <= 0 && (string) $dt->posisi_dc === 'kredit') {
                ...
                JurnalDetailORM::create([
                    ...
                    'debit'            => $debit,
                    'kredit'           => $kredit,
                    'keterangan_baris' => ($dt->keterangan_baris ?? $template->nama_template_jurnal) . ' - ' . $row->no_biaya_produksi,
                    'id_pelanggan'     => null,
                    'id_pemasok'       => null,
                    'id_produk'        => null,
                    'id_bahan_baku'    => null,
        ...
        LogJurnalSumberORM::create([
            ...
            'id_sumber'              => $id_biaya_produksi,
            'no_sumber'              => $row->no_biaya_produksi,
            'kode_jenis_transaksi'   => count(array_unique($kode_log)) === 1 ? $kode_log[0] : 'BIAYA_PRODUKSI',
            'tanggal_dibuat'         => date('Y-m-d H:i:s'),
        ]);
    ...
    set_flash('success', 'Biaya produksi berhasil diposting dan jurnal otomatis sudah dibuat dari template database.');
    header('Location: ' . admin_url('index.php?menu=produksi/biaya/detail&id=' . $id_biaya_produksi . '&back_url=' . urlencode($back_url)));
    exit;
} catch (Throwable $e) {
