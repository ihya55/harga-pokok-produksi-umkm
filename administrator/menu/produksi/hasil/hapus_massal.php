...
if (!is_array($ids) || count($ids) === 0) {
    set_flash('error', 'Pilih data hasil produksi yang akan dihapus.');
...
if (count($ids) === 0) {
    set_flash('error', 'ID hasil produksi tidak valid.');
    header('Location: ' . $back_url);
    exit;
}
...
try {
    ...
    Capsule::connection()->transaction(function () use (
        ...
        foreach ($ids as $id_hasil_produksi) {
            $row = HasilProduksiORM::query()
                ->where('id_entitas', $id_entitas)
                ->find((int) $id_hasil_produksi);
    ...
    if ($jumlah_terhapus > 0 && $jumlah_dilewati > 0) {
        ...
        set_flash('success', 'Data hasil produksi terpilih berhasil dihapus.');
    } else {
        set_flash('error', 'Tidak ada data yang dihapus. Hanya draft yang bisa dihapus.');
    }
} catch (Throwable $e) {
    set_flash('error', 'Gagal hapus massal hasil produksi: ' . $e->getMessage());
}
